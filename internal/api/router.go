package api

import (
	"net/http"

	"github.com/muonsoft/errors"
)

// NewMux создаёт ServeMux с зарегистрированными маршрутами (Go 1.22+).
// auth — опционально; при nil auth endpoints не регистрируются.
func NewMux(h *Handler, auth *AuthHandler) (*http.ServeMux, error) {
	mux := http.NewServeMux()
	mux.HandleFunc("GET /healthz", func(w http.ResponseWriter, _ *http.Request) { w.WriteHeader(http.StatusOK) })
	mux.HandleFunc("GET /readyz", func(w http.ResponseWriter, _ *http.Request) { w.WriteHeader(http.StatusOK) })
	if auth != nil {
		mux.HandleFunc("POST /api/auth/login", auth.Login)
		mux.HandleFunc("GET /api/auth/session", auth.Session)
		mux.HandleFunc("POST /api/auth/logout", auth.Logout)
		mux.HandleFunc("GET /api/auth/google", auth.GoogleOAuthStart)
		mux.HandleFunc("GET /api/auth/google/callback", auth.GoogleOAuthCallback)
		mux.HandleFunc("GET /api/auth/yandex", auth.YandexOAuthStart)
		mux.HandleFunc("GET /api/auth/yandex/callback", auth.YandexOAuthCallback)
	}
	mux.HandleFunc("GET /api/nodes/by-id/{id}", h.GetNodeByID)
	mux.HandleFunc("GET /api/nodes/{path...}", h.GetNode)
	mux.HandleFunc("POST /api/nodes/update", h.UpdateNode)
	mux.HandleFunc("POST /api/nodes/delete", h.DeleteNode)
	mux.HandleFunc("POST /api/nodes/move", h.MoveNode)
	mux.HandleFunc("POST /api/nodes/refresh-description", h.RefreshDescription)
	mux.HandleFunc("POST /api/nodes/normalize", h.PostNodeNormalize)
	mux.HandleFunc("POST /api/nodes/agent-edit", h.PostNodeAgentEdit)
	mux.HandleFunc("POST /api/nodes/dump-images", h.PostNodeDumpImages)
	mux.HandleFunc("GET /api/nodes", h.ListNodes)
	mux.HandleFunc("GET /api/label-suggestions", h.GetLabelSuggestions)
	mux.HandleFunc("GET /api/assets/{path...}", h.GetAsset)
	mux.HandleFunc("GET /api/tree", h.GetTree)
	mux.HandleFunc("GET /api/search", h.Search)
	mux.HandleFunc("POST /api/search", h.PostSearch)
	mux.HandleFunc("POST /api/ingest", h.Ingest)
	mux.HandleFunc("GET /api/git/status", h.GetGitStatus)
	mux.HandleFunc("POST /api/git/commit", h.PostGitCommit)
	mux.HandleFunc("POST /api/git/sync", h.PostGitSync)
	mux.HandleFunc("POST /api/import/telegram", h.ImportTelegramCreate)
	mux.HandleFunc("GET /api/import/telegram/session/{id}", h.ImportTelegramGetSession)
	mux.HandleFunc("POST /api/import/telegram/session/accept", h.ImportTelegramAccept)
	mux.HandleFunc("POST /api/import/telegram/session/reject", h.ImportTelegramReject)
	mux.HandleFunc("POST /api/articles/translate", h.PostArticleTranslate)
	mux.HandleFunc("GET /api/articles/translate/{path...}", h.GetArticleTranslate)
	mux.HandleFunc("GET /api/chats", h.GetChats)
	mux.HandleFunc("POST /api/chats", h.PostChats)
	mux.HandleFunc("GET /api/chats/{id}", h.GetChatByID)
	mux.HandleFunc("POST /api/chats/update", h.UpdateChat)
	mux.HandleFunc("POST /api/chats/delete", h.DeleteChat)
	mux.HandleFunc("POST /api/chat", h.PostChat)
	mux.HandleFunc("POST /api/debug/issues", h.PostDebugIssue)
	mux.HandleFunc("POST /api/debug/issues/update", h.UpdateDebugIssueStatus)
	mux.HandleFunc("POST /api/index/rebuild", h.PostIndexRebuild)
	mux.HandleFunc("GET /api/index/status", h.GetIndexStatus)
	mux.HandleFunc("POST /api/jobs", h.PostJobs)
	mux.HandleFunc("GET /api/jobs/{id}", h.GetJobStatus)
	mux.HandleFunc("GET /api/jobs/{id}/logs", h.GetJobLogs)
	mux.HandleFunc("GET /api/node-normalization/{id}", h.GetNodeNormalizeStatus)
	mux.HandleFunc("GET /api/node-normalization/{id}/logs", h.GetNodeNormalizeLogs)
	mux.HandleFunc("GET /api/node-agent-edit/{id}", h.GetNodeAgentEditStatus)
	mux.HandleFunc("GET /api/node-agent-edit/{id}/logs", h.GetNodeAgentEditLogs)
	mux.HandleFunc("GET /api/node-dump-images/{id}", h.GetNodeDumpImagesStatus)
	mux.HandleFunc("GET /api/node-dump-images/{id}/logs", h.GetNodeDumpImagesLogs)
	spa, err := NewSPAHandler()
	if err != nil {
		return nil, errors.Errorf("new spa handler: %w", err)
	}
	mux.Handle("GET /{$}", spa)
	mux.Handle("GET /index.html", spa)
	mux.Handle("GET /{path...}", spa)

	return mux, nil
}
