...
// SearchVocabularyOptions controls search rewrite vocabulary selection.
...
// SearchVocabulary returns compact curated terms useful for LLM query rewriting.
func (s *IndexStore) SearchVocabulary(ctx context.Context, opts SearchVocabularyOptions) ([]string, error) {
...
func normalizeSearchVocabularyOptions(opts SearchVocabularyOptions) SearchVocabularyOptions {
	if opts.Limit <= 0 {
		opts.Limit = 150
	}
...
func validVocabularyTerm(term string, opts SearchVocabularyOptions) bool {
	...
	if _, ok := keywordStopWords[term]; ok {
		return false
	}
