...
func (s *Store) CreateSession(ctx context.Context, id, title string) (chat.Session, error) {
	now := time.Now().UTC()
	...
	return chat.Session{ID: id, Title: title, CreatedAt: now, UpdatedAt: now}, nil
}

func (s *Store) ListSessions(ctx context.Context) ([]chat.Session, error) {
	rows, err := s.db.QueryContext(ctx, `SELECT id, title, created_at, updated_at FROM chat_sessions ORDER BY updated_at DESC`)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	items := make([]chat.Session, 0)
	for rows.Next() {
		var i chat.Session
...
func (s *Store) GetSession(ctx context.Context, id string) (chat.SessionDetails, error) {
	out := chat.SessionDetails{
