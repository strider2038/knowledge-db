...
- Если вход — длинный пересланный/авторский текст (например, из Telegram) и в нём есть URL как ссылка по теме, это всё равно type: note; не понижать в link только из-за наличия URL.
...
- Для profile link (content_profile != link_bookmark) поле content обязательно и должно содержать markdown digest; пустой content недопустим.
...
- slug: СТРОГО ASCII kebab-case [a-z0-9-] (без кириллицы, пробелов, процентов, подчеркиваний, точек и других символов); обязательно транслитерируй
...
- Выбирай theme_path по ПРЕДМЕТУ материала: о чём именно этот узел, а не откуда он пришёл и в каком формате сохранён.
- source_kind, content_profile и type — диагностические свойства источника, а не тематические сигналы. Не помещай материал в ветку только из-за совпадения этих полей.
- Candidate themes — подсказки, а не обязательный выбор. Предпочитай candidate theme только если её предмет действительно совпадает с материалом.
- Предпочитай наиболее точную существующую дочернюю тему широкой родительской категории. Не используй catch-all тему только потому, что в ней уже много узлов.
- Ветка agentic coding подходит для coding agents, их workflows и инструментов разработки. Память агентов, RAG/retrieval, MCP/integrations, модели и обычные online services — отдельные предметы, если материал не связывает их именно с coding agents.
- Если candidates описывают лишь смежную область, вызови search_placement_candidates или выбери более точную тему из theme map.
- Предпочитай candidate keywords, если они подходят по смыслу.
...
- В theme_path и slug используй только ASCII: каждый сегмент пути и slug должны содержать только [a-z0-9-].
