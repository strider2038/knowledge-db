...
- source_url: для type: article и type: link — ВСЕГДА URL ресурса (статья, сервис, инструмент), который ты передаёшь в fetch_url_content или fetch_url_meta. НЕ подставляй ссылки из тела статьи (цитаты, сноски, примеры вроде claude.md или GitHub) — это не URL публикации. НЕ используй ссылки на мессенджеры (t.me, telegram.org) — они лишь канал доставки. Если контент пришёл из пересланного сообщения, бери URL из текста сообщения
