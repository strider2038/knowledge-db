...
- **WHEN** задана любая, но не вся, группа Google-OAuth-переменных (включая «висящие» state/allowlist только под Google)
...
### Requirement: Конфликт конфигурации парольного и OAuth-режимов

**REMOVED** как отдельное требование взаимоисключения пароля и OAuth. Пароль и OAuth MUST сочетаться, если оба полностью настроены.

**Migration:** инстансы, которые раньше держали пустые `KB_LOGIN`/`KB_PASSWORD` «чтобы включить Google», могут оставить их пустыми; при необходии пароля — задать оба поля без отключения OAuth.
