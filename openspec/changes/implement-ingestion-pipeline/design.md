...
2. Если merge conflict → логировать warning, оставить конфликт для ручного разрешения пользователем
...
**[Git конфликты при sync]** → Сервер на VPS + локальная работа через Cursor = два источника коммитов. Конфликты маловероятны (обычно разные файлы), но при одновременном редактировании одного узла — merge conflict. Стратегия: логировать warning, оставить для ручного разрешения. Не автоматически резолвить.
