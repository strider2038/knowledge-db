...
Структура хранения — плоская: каждый узел хранится как файл `{slug}.md` непосредственно в директории темы (без slug-поддиректории). Файл содержит YAML frontmatter и markdown-тело. Дополнительно рядом с файлом узла может существовать директория `{slug}/` для вложений (images, notes) — она не считается подтемой. Директория `.local/` внутри вложений исключена из git.
