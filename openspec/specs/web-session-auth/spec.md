...
Опциональная авторизация для web-доступа к knowledge-db: пароль (`KB_LOGIN` / `KB_PASSWORD`) или Google OAuth 2.0 с allowlist email, серверные cookie-сессии.
...
- **THEN** сервер SHALL создать cookie-сессию и завершить OAuth-обмен редиректом в веб-интерфейс
...
В **парольном** режиме сервер MUST предоставлять `POST /api/auth/login`, `POST /api/auth/logout`, `GET /api/auth/session` так же, как в базовом сценарии пароля. В **Google-режиме** сервер MUST предоставлять `GET /api/auth/google`, `GET /api/auth/google/callback`, `POST /api/auth/logout`, `GET /api/auth/session`, а `POST /api/auth/login` SHALL NOT создавать сессию. `GET /api/auth/google` SHALL инициировать OAuth, `GET /api/auth/google/callback` SHALL обменивать `code` на токены и, при допустимом email, устанавливать cookie. `GET /api/auth/session` SHALL возвращать `authenticated` и, при применимости, `auth_enabled` для веб-клиента. `POST /api/auth/logout` SHALL инвалидировать сессию и очищать cookie.
...
- **WHEN** в парольном режиме учётные данные неверны
