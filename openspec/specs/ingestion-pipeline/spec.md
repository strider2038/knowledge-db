...
- **WHEN** pipeline обрабатывает материал через IngestText с source_url, для которого уже существует другой узел с тем же нормализованным source_url (любой type)
...
- **WHEN** ingestion выполняет update существующего узла по source_url при явном ingest URL
