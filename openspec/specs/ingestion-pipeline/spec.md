...
Операция RefreshDescription ДОЛЖНА (SHALL) учитывать текущий тип и профиль узла, чтобы не деградировать digest-контент.
...
Для профильных link-узлов (`type=link` и `content_profile` задан и не равен `link_bookmark`) поле `content` MUST быть непустым markdown digest после оркестрации. Если оркестратор вернул пустой `content`, система MUST выполнить один retry с явным требованием непустого digest; если `content` остаётся пустым, refresh MUST завершиться ошибкой и MUST NOT мутировать существующий файл узла.
...
- **THEN** pipeline выполняет один retry с усиленной инструкцией; при повторно пустом `content` возвращает ошибку без изменения файла
